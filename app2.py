...
plant_list = ['Not_Detected', 'Tomato_healthy', 'Tomato_unhealthy_late_blight','Tomato_unhealthy_leaf_miner_diseases','Tomato_unhealthy_septoria_leaf_spot']
...
try:
    while True:
        ...
        if "unhealthy" in predicted_class:  # Example: water if plant is not healthy
            print(f"[ACTION] {predicted_class} detected. Spray chlorothalonil")
            GPIO.output(RELAY_GPIO_PIN, GPIO.LOW)
            time.sleep(PUMP_ON_DURATION)
            GPIO.output(RELAY_GPIO_PIN, GPIO.HIGH)
            print("[ACTION] pump turned off.")
